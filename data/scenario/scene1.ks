;=============================================
; Re:verse - 第一章「揺籃」
; シーン1: 朝の執務室 / シーン2: 朝食の間
;=============================================

[cm]
[clearfix]
[tb_hide_message_window]

;--- シーン1: 朝の執務室 ---
[bg storage="bg_office.png" time="1500"]
[wait time="500"]
[playbgm volume="30" time="1000" loop="true" storage="bgm_daily.mp3"]
[wait time="500"]

[tb_show_message_window]

[tb_start_text mode=1]
#
私は強くなくてはならない。誰にも頼ることなく。[p]
[_tb_end_text]

[tb_start_text mode=1]
#
朝の光が執務室の窓から差し込む。[r]
埃が光の筋の中を舞っている。[r]
子供の頃はこれを「妖精の行進」と呼んでいた。[r]
今の私にはただの粉塵だ。[p]
[_tb_end_text]

[tb_start_text mode=1]
#
机の上には昨夜から積み上がった書類が山をなしている。[r]
税収の減少報告。領民の流出記録。[r]
ロボット導入に伴う雇用問題。借入金の返済催促。[r]
どれも、この家の行く末を暗示するようなものばかりだ。[p]
[_tb_end_text]

[tb_start_text mode=1]
#
子供の頃からお父様に君主論と帝王学を厳しく叩き込まれてきた。[r]
「民は羊ではない。だが、導く者がいなければ崖から落ちる」[r]
お父様の口癖だった。[r]
今では領政の片腕として、それなりにお役に立てるようになったと思う。[p]
[_tb_end_text]

[tb_start_text mode=1]
#
……けれど、崖から落ちかけているのは、導く側のこの家なのかもしれない。[p]
[_tb_end_text]

;--- 侍女登場 ---
[playse volume="80" storage="se_knock.mp3"]
[wait time="500"]

[tb_start_text mode=1]
#侍女
「お嬢様、朝食の準備が整いました」[p]
[_tb_end_text]

[tb_start_text mode=1]
#
「ありがとう。すぐに行くわ」[p]
[_tb_end_text]

[tb_start_text mode=1]
#
侍女は私が生まれた時からずっと側にいてくれる人だ。[r]
歳は私より五つ上で、お姉さんのような存在でもある。[r]
……もっとも、この家で「お姉さん」と甘えることは許されていないのだけれど。[p]
[_tb_end_text]

;=============================================
; シーン2: 朝食の間
;=============================================

[tb_hide_message_window]
[mask time="1500" effect="fadeIn" color="0x000000"]
[wait time="500"]
[bg storage="bg_dining.png" time="0"]
[wait time="500"]
[mask_off time="1500" effect="fadeOut"]
[wait time="500"]
[tb_show_message_window]

[tb_start_text mode=1]
#
父が先に席についている。[r]
いつものように背筋を伸ばし、新聞に目を通している。[r]
――いや、今朝は新聞ではなく、何通もの封書を広げている。[p]
[_tb_end_text]

[tb_start_text mode=1]
#
その表情が険しい。封書には見慣れた紋章がいくつも押されていた。[r]
債権者たちからの催促だろう。[p]
[_tb_end_text]

[tb_start_text mode=1]
#
「おはようございます、お父様」[p]
[_tb_end_text]

[tb_start_text mode=1]
#父
「……座りなさい」[p]
[_tb_end_text]

[tb_start_text mode=1]
#父
「北部の村で、また耕作ロボットが止まった。整備費用が捻出できない」[p]
[_tb_end_text]

[tb_start_text mode=1]
#
「……南部の畑は」[p]
[_tb_end_text]

[tb_start_text mode=1]
#父
「人手不足で収穫が追いつかない。若い者は皆、都市へ出て行った」[p]
[_tb_end_text]

[tb_start_text mode=1]
#
沈黙が落ちる。[r]
この半年で領地の人口は二割近く減った。[r]
テクノロジーの波は豊かな都市へ人を集め、[r]
旧い領地からは命の水のように人が流れ出ていく。[p]
[_tb_end_text]

[tb_start_text mode=1]
#父
「……今日の午後、話がある。私の部屋に来なさい」[p]
[_tb_end_text]

[tb_start_text mode=1]
#
「……はい」[p]
[_tb_end_text]

[tb_start_text mode=1]
#
父の声に、いつもと違う重みがあった。[r]
封書の束を片付ける父の手が、わずかに震えているのが見えた。[p]
[_tb_end_text]

;--- 第二章へ ---
[jump storage="scene2.ks"]
